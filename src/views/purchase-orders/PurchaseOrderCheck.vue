...
const getPurchase = async () => {
  const { data: response } = await purchaseOrderService.getPurchaseOrder(
    route.params.id
  );
      ...
      <form>
        <div class="table-responsive">
          <table class="table table-striped table-hover table-bordered">
            ...
            <tbody>
              <tr
                v-for="(purchaseDetail, i) in purchase.purchase_order_details"
              >
                <td>{{ i + 1 }}</td>
                <td>{{ purchaseDetail.product.name }}</td>
                <td>{{ purchaseDetail.quantity_demanded }}</td>
                <td>{{ purchaseDetail.quantity_received }}</td>
                <td>
                ...
                <td class="text-center">
                  <label class="form-check">
                    <input
                      @change="verifyProduct(purchaseDetail.product.id)"

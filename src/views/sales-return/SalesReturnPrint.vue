    ...
    <div id="printable">
      <Panel>
        ...
        <div class="row">
          ...
          <div class="col-4">
            <h2 class="fw-bold">Sale Return</h2>
            <p class="mb-0">Invoice No: {{ saleReturn.id }}</p>
            <p>
              Billing Date: {{ moment(saleReturn.date).format("DD MMMM YYYY") }}
            </p>
            <span class="badge bg-success"><strong>Billing To</strong></span>
            <h3 class="fw-bold mt-2">{{ saleReturn.sale.account.name }}</h3>
          </div>
        </div>
        <hr />
        <div class="table-responsive mt-4">
          <table class="table table-bordered">
            ...
            <tbody>
              <tr v-for="(sale_details, i) in saleReturn.sales_return_details">
                <td>{{ i + 1 }}</td>
                <td>{{ sale_details.product.name }}</td>
                <td>{{ sale_details.quantity_return }}</td>
                <td>{{ sale_details.price }}</td>
                <td>{{ sale_details.price * sale_details.quantity_return }}</td>
              </tr>
            </tbody>
          </table>
        </div>
        <div class="row mt-5">
          ...
          <div class="col">
            <div class="mb-3 d-flex justify-content-between mb-3">
              ...
              >
              <b>{{ saleReturn.sale_return_amount }}</b>
            </div>
          </div>
        </div>
      </Panel>
